...
d3.csv("covid_2023_ref2.csv").then(function(data) {
    ...
    var regiones = [...new Set(data.map(d => d.Nombre_departamento))];

    // Crear la lista desplegable con el nombre de las regiones
    var selectRegion = d3.select("#regionSelect")
        .append("select")
    ...
    function updateTable() {
        ...
        var datosFiltrados2 = data.filter(function(d) {
            return d.Muerte_covid === filtro && d.Nombre_departamento === filtro2;
        });
