...
//       .attr("stroke-width", 1)
//       .attr("opacity", 0.7)
//       // Paso 1: Agregar eventos de hover
//       .on("mousemove", function (event, d) {
//         const coords = d[0].split(",");
//         const totalCasesInArea = d[1];
//         const department = departmentsByCoordinates[d[0]];
//         const percentage = ((totalCasesInArea / totalCases) * 100).toFixed(2); // Calcula el porcentaje de casos
//         // Paso 2: Mostrar información de casos en hover
//         showTooltip(
//           event.pageX,
//           event.pageY,
//           department,
...
//         // Paso 3: Ocultar la información de casos
//         hideTooltip();
//       });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateVisualizations(covidData, geoData) {
    ...
    const container = d3.select("#mapa"); // Selecciona el contenedor con el id #mapa

    const svg = container
      .append("svg") // Añade un SVG al contenedor seleccionado
      .attr("width", width)
      .attr("height", height);
